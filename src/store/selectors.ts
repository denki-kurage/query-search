...
// 全ての結果ビューを取得します。
export const getAllResultView = createRegistrySelector(select => (state: StateType) =>
{
	const { findBlocks } = select(store);
	const rv = select(store).getResultViewName();
	return findBlocks().filter(b => b.name === rv);
});
...
{
	return select(store).getAllResultView().find(rv => rv.attributes.uid === resultViewId);
});
...
export const getAllEndpointArgs = createRegistrySelector(select => (state: StateType, kind: string, name: string) => {
	const option = state.schemas.options?.[[kind, name].join(':')];
	const argsItems = option?.endpoints?.filter(ep => ep.methods.includes('GET')).map(ep => ep.args);
